...
class account_voucher(osv.osv):
    ...
    def onchange_date_604(self, cr, uid, ids, partner_id, journal_id, price, currency_id, ttype, date, context=None):
        res = super(account_voucher, self).onchange_date(cr, uid, ids, partner_id, journal_id, price, currency_id, ttype, date, context=context)
        res.setdefault("value", {})["fecha_trans"] = date
        return res
    def onchange_date_610(self, cr, uid, ids, date, currency_id, payment_rate_currency_id, amount, company_id, ttype=None, context=None):
    ...
    def onchange_journal_610(self, cr, uid, ids, journal_id, line_ids, tax_id, partner_id, date, amount, ttype, company_id, context=None):
    ...
    if release.version != '6.1':
        onchange_date = onchange_date_604
    else:
        onchange_date = onchange_date_610
        onchange_journal = onchange_journal_610
                ...
                for move_line in rec.move_id.line_id:
                    ...
                    if rec.journal_id.currency and rec.journal_id.currency.name != "MXN":
                        ...
                        if cur_obj.read(cr, uid, currency_id, ["base"])["base"]:
                            ...
                            rate_other = cur_obj._current_rate(cr, uid, [currency_id], "rate", [], context=ctx)[currency_id]
                        #rate_mxn = cur_obj._current_rate_computation(cr, uid, [mxn_currency_id], "rate", [], True, context=ctx)[mxn_currency_id]
                        rate_mxn = cur_obj._current_rate(cr, uid, [mxn_currency_id], "rate", [], context=ctx)[mxn_currency_id]
    ...
    _columns = {
        ...
        "num_cheque": fields.char(u"Número", size=64, ),
        "benef": fields.many2one("res.partner", string="Beneficiario"),
        "metodo_pago": fields.many2one("contabilidad.electronica.metodo.pago", string=u"Código"),
        "tipo_pago": fields.selection([
        		("trans", "Transferencia"),
        		("cheque", "Cheque"), 
        		("otro", "Otro")], string=u"Tipo del pago")
    }
    _defaults = {
        "tipo_pago": 'trans',
